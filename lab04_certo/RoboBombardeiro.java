...
class RoboBombardeiro extends RoboTerrestre implements Sensoreavel, Atacante {
    ...
    public void plantarBombaNoLocal() throws RoboDesligadoException, AcaoNaoPermitidaException {
        ...
        if (quantidadeBombas > 0) {
            System.out.println("Robô " + retornarNome() + " plantou uma bomba na posição (" + retornarX() + ", " + retornarY() + ", " + getZ() + ").");
            quantidadeBombas--;
            // ambiente.adicionarEntidade(new Bomba(retornarX(), retornarY(), getZ()));
        } else {
    ...
    public void acionarSensores(Ambiente ambiente) throws RoboDesligadoException {
        ...
        System.out.println(retornarNome() + " (Bombardeiro) usando sensores para identificar alvos estratégicos.");
    }
